...
class Bip39:
    ...
    def mnemonics_to_seed(mnemonics: List[str], passphrase: str = '') -> bytes:
        ...
        chksum: bytes = entropy_and_chksum[-1:]  # 8-bit chksum
        # Verify chksum equals to the first byte of SHA256(entropy)
        chksum2 = hashlib.sha256(entropy).digest()
        assert chksum == chksum2[0:1]
